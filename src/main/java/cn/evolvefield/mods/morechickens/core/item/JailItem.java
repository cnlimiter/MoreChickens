...
public class JailItem extends Item {
    ...
    public ActionResultType useOn(ItemUseContext context) {
        ...
        if(!(tileEntity instanceof NestTileEntity)) { // When using to release a quail
            if (jailTag == null)
                return ActionResultType.PASS;
            BaseChickenEntity released = (BaseChickenEntity) ModDefaultEntities.BASE_CHICKEN.get().spawn((ServerWorld) world, itemStack, player, context.getClickedPos().relative(context.getClickedFace()), SpawnReason.SPAWN_EGG, true, false);
            if (released != null)
